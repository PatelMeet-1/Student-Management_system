...
export default function InternalMarksManager() {
  ...
  const loadResults = async () => {
    try {
      const res = await axios.get(`${API}?type=internal`);
      const data =
          ...
          if (sem?.subjects) {
            setSubjects(
              sem.subjects.map((s) => ({
                ...
                maxMarks: s.internalTotal || s.maxMarks || 30,
              }))
            );
            return;
          }
  ...
  const submitExcel = async () => {
    ...
    try {
      const existingRes = await axios.get(`${API}?type=internal`);
      const existingData =
  ...
  // ✅ REAL-TIME MARKS VALIDATION FOR EDIT MODE (12/30 = 40%)
  ...
  const addSubject = async () => {
    ...
    const validation = validateMarks(newSubject.marks, 30);
    if (!validation.valid) {
  ...
  return (
    <div className="container mt-4">
      <ToastContainer />
      <h3 className="text-center mb-4">🎓 Internal Marks Manager</h3>
      ...
      <div className="card p-3 mb-4">
        ...
        {subjects.length > 0 && (
          <div className="alert alert-success mt-2">
            ...
            <span className="badge bg-warning ms-2">12/30 Pass (40%)</span>
          </div>
        )}
      </div>
                ...
                <tbody>
                  {editingSubjects.map((s, i) => (
                    <tr key={i}>
                      ...
                      {/* ✅ INDIVIDUAL SUBJECT PASS: 12/30 = 40% */}
                      <td>
                        <span
                          className={`badge px-2 py-1 ${
                            (s.marks / s.maxMarks) * 100 >= 40
                              ? "bg-success"
                              : "bg-danger"
                          }`}
                        >
