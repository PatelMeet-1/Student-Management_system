...
export default function PracticalMarksManager() {
  ...
  const loadResults = async () => {
    try {
      const res = await axios.get(`${API}?type=practical`);
      const data =
  ...
  // ---------------- SUBJECTS FROM COURSE STRUCTURE ----------------
  useEffect(() => {
          ...
          if (sem?.subjects) {
            setSubjects(
              sem.subjects.map((s) => ({
                ...
                maxMarks: s.practicalTotal || s.maxMarks || 25,
              }))
            );
            return;
          }
  ...
  const calculateStatus = (subjects) => {
    ...
    const hasFail = subjects.some(
      (s) => ((s.marks || 0) / (s.maxMarks || 1)) * 100 < PASSING_PERCENTAGE
    );
    return hasFail ? "❌ FAIL" : "✅ PASS";
  };

  // ✅ MARKS VALIDATION FUNCTION (UNCHANGED)
  ...
  // ✅ VALIDATED EXCEL SUBMIT WITH MAX MARKS CHECK (UPDATED PASSING CHECK)
  const submitExcel = async () => {
    ...
    try {
      const existingRes = await axios.get(`${API}?type=practical`);
      const existingData =
      ...
      for (let i = 0; i < excelRows.length; i++) {
        try {
          ...
          uploaded++;
          localResults.push({ data: { data: { Sem: uniqueSem, studentId: student._id } } });
        } catch (err) {
  ...
  // ✅ REAL-TIME MARKS VALIDATION FOR EDIT MODE (UPDATED PASSING CHECK)
  ...
  const addSubject = async () => {
    ...
    const validation = validateMarks(newSubject.marks, 25);
    if (!validation.valid) {
  ...
  return (
    <div className="container mt-4">
      <ToastContainer />
      <h3 className="text-center mb-4">🧪 Practical Marks Manager</h3>
              ...
              <table className="table table-bordered">
                <thead className={editMode ? "table-warning" : "table-primary"}>
                  <tr>
                    ...
                    <th>Max</th>
                    <th>%</th>
                    {/* ✅ UPDATED PASSING THRESHOLD DISPLAY */}
                    <th>Grade ({PASSING_PERCENTAGE}%)</th>
                      ...
                      <td>
                        <span
                          className={`badge px-2 py-1 ${
                            (s.marks / s.maxMarks) * 100 >= PASSING_PERCENTAGE
                              ? "bg-success"
                              : "bg-danger"
                          }`}
                        >
                          {(s.marks / s.maxMarks) * 100 >= PASSING_PERCENTAGE ? "Pass" : "Fail"}
                        </span>
                      </td>
                      {editMode && (
                        <td>
