...
export default function StudentResult({ loggedUser, setError, API }) {
  const [result, setResult] = useState(null);

  const fetchResult = async () => {
    try {
      const res = await fetch(`${API}/users`); // ✅ use API from .env
      const data = await res.json();
      const student = data.find(u => String(u._id) === String(loggedUser._id));
  ...
  const totalMarks = result?.results?.reduce((sum, r) => sum + r.marks, 0) || 0;
  const maxMarks = result?.results?.length ? result.results.length * 100 : 0;
        ...
        <div id="printResult">
          <Card className="p-4 shadow">
            ...
            <div className="text-center mb-3">
              {result.photo && (
                <img
                  src={`${API}${result.photo}`} // ✅ use API here as well
                  alt="Student"
